...
def run_truss_2D_bar(n_elements: int, end_point: tuple,force_vector: tuple,label: str):
    """
    ...
    xm = xn/2-0.1
    ym = yn/2+0.1
    x1 = np.linspace(0,xm,int(n_elements/2)+1)
    x2 = np.linspace(xm,xn,int(n_elements/2)+1)
    x = np.hstack((x1[:-1],x2))
    y1 = np.linspace(0,ym,int(n_elements/2)+1)
    y2 = np.linspace(ym,yn,int(n_elements/2)+1)
    y = np.hstack((y1[:-1],y2))

    # a, b, c = 1.7, -0.7, 1.83848 #arc parameters
    # x = np.linspace(0,xn,n_elements+1)
    # y = np.sqrt(c**2 - (x-a)**2) + b
    ...
    bcs = (
        [
            ...
            DirichletBC(bc_type = BCType.NODE, component = 0, index = n_elements, value = 0.0),
            DirichletBC(bc_type = BCType.NODE, component = 1, index = n_elements, value = 0.0),
    ...
    plt.plot(*(points).T,marker='d')
    plt.plot(*(u_truss+points).T,marker='d')
    plt.axis('equal')
    plt.show()
    return u_truss, points
