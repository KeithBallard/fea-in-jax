...
def test_multiscale_prnn_rectangle_solve():
    ...
    dirichlet_values = np.concatenate(dirichlet_values)
    ...
    for i, scale_factor in enumerate(np.linspace(0, 1, 10)):
        scaled_dirichlet_values = scale_factor * dirichlet_values
        boundary_conditions = [
            DirichletBC(index=int(node), component=int(component), value=float(value))
            for (node, component), value in zip(dirichlet_bcs, scaled_dirichlet_values)
        ]
        ...
        u, residual, new_internal_state_beqi = solve_bvp(
            ...
            solver_options=SolverOptions(linear_solve_type=LinearSolverType.DENSE_INVERSE_JNP),
        )
