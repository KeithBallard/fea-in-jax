...
def test_cg_solver_variants_solve_constrained_linear_system():
    x0 = jnp.array([0.0, 0.0, 0.0], dtype=dtype)
    b = jnp.array([2.0, 5.0, 1.0], dtype=dtype)
    tol = 1e-12
    atol = 0.0
    expected_solution = jnp.array([3.0, 4.0, 1.0], dtype=dtype)
    ...
    for solver_type in SolverType:
        ...
        print("R = ", r_solution)
        assert jnp.linalg.norm(r_solution) < 1e-5
        assert jnp.allclose(x_solution, expected_solution, rtol=1e-5, atol=1e-5)
