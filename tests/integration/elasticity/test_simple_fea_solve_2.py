...
def test_simple_fea_solve_center_node_mesh():
    # Make the mesh:
    #    <-----3----->
    #  2 o-----------o 1 ^
    #    |  \  2  /  |   |
    ...
    boundary_conditions = [
        DirichletBC(index=int(node), component=int(component), value=float(value))
        for (node, component), value in zip(dirichlet_bcs, dirichlet_values)
    ]
    print("dirichlet_bcs = \n", dirichlet_bcs)
    ...
    element_batches = [
        ElementBatch(
            ...
            material_params=mat_params_eqp[:1,:]
        ),
        ElementBatch(
            fe_type=fe_type,
            n_dofs_per_basis=U,
    ...
    expected_lateral_strain_ratio = 0.25 / (1.0 - 0.25)
    assert (u[0] - 1.0) < 1e-6 and (u[2] - 1.0) < 1e-6
    assert abs(-(u[3] / 2.0) / (u[0] / 3.0) - expected_lateral_strain_ratio) < 1e-6
    assert abs(-(u[5] / 2.0) / (u[0] / 3.0) - expected_lateral_strain_ratio) < 1e-6
    assert u[1] == u[7] == 0.0
    assert u[4] == u[6] == 0.0
