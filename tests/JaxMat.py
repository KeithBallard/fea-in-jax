...
class JaxMat:
    def __init__(self, dmplex, jaxMult):
        self.dmplex = dmplex
        self.localX = dmplex.createLocalVec() if dmplex is not None else None
        self.jaxMult = jaxMult

    def mult(self, A: Mat, x: Vec, y: Vec) -> None:
        """Matrix vector multiplication: y = A @ x."""
        # This first smoke-test path intentionally copies through host memory.
        # Once the call stack is proven, this is the boundary to replace with
        # DLPack/CUDA-handle plumbing.
        input_vec = jnp.asarray(jnp.asarray(x.getArray(readonly=True)))
        output = self.jaxMult(input_vec)
        output.block_until_ready()
        y_array = y.getArray(readonly=False)
        y_array[...] = jnp.asarray(output)
