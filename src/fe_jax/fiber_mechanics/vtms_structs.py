...
class VTMSBundle:
    ...
    def get_fiber_n_elements(self, bundle_i: int, fiber_i: int):
        return (
            self.fiber_offsets[fiber_i + 1]
            - self.fiber_offsets[fiber_i]
            - 1
        )

    def get_n_elements(self):
        n_elements = 0
        for i in range(self.fiber_offsets.shape[0] - 1):
            n_elements += self.fiber_offsets[i + 1] - self.fiber_offsets[i] - 1
        return n_elements
...
def read_fabric(filepath: str | Path) -> VTMSFabric:
    ...
    return VTMSFabric(
        ...
        fiber_offsets=np.hstack(
            [
                b.fiber_offsets[0:-1] + bundle_point_offsets[i]
                for i, b in enumerate(bundles)
            ]
            + [bundle_point_offsets[-1]]
        ),
        bundle_offsets=np.append([0], np.cumsum([b.n_fibers for b in bundles])),
    )
